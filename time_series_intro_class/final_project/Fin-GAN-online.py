...
def Evaluation2(ticker, freq, gen, test_data, val_data, h, l, pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, losstype,
...
def Evaluation3(tickers, freq, gen, test, val, h, l, pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, losstype, sr_val,
                device, plotsloc, f_name, plot=False):
    """
...
def GradientCheck(ticker, gen, disc, gen_opt, disc_opt, criterion, n_epochs, train_data, batch_size, hid_d, hid_g,
...
def TrainLoopForGAN(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
...
def TrainLoopMainPnLnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
...
def TrainLoopMainPnLMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs,
...
def TrainLoopMainPnLMSESRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                            checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                            lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            gen_loss = criterion(disc_fake_pred,
                                 torch.ones_like(disc_fake_pred)) - alpha * PnL + beta * SqLoss - gamma * SR
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
...
def TrainLoopMainPnLMSESTDnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs,
...
def TrainLoopMainPnLSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                         train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                         l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopMainMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                       train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                       l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopMainSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                      train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10,
                      pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopMainSRMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                         train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                         l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopMainPnLSTDnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                          checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                          lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def FinGAN_combos(ticker, loc, modelsloc, plotsloc, dataloc, etflistloc, vl_later=True, lrg=0.0001, lrd=0.0001,
    ...
    gen, disc, gen_opt, disc_opt, alpha, beta, gamma, delta = GradientCheck(ticker, gen, disc, gen_opt, disc_opt,
    ...
    df_temp, PnL_test[0], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnL,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[1], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLMSE,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[2], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLMSESTD,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[3], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLMSESR,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[4], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLSR,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[5], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLSTD,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[6], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genSR,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[7], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genSRMSE,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[8], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genMSE,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    df_temp, PnL_test[9], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genfg,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
...
def GradientCheckLSTM(ticker, gen, gen_opt, n_epochs, train_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                      lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def Evaluation2LSTM(ticker, freq, gen, test_data, val_data, h, l, pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs,
                    losstype, sr_val, device, plotsloc, f_name, plot=False):
    """
...
def TrainLoopnLSTMPnL(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                      validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                      diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopnLSTMPnLSTD(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                         validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                         diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopnLSTMPnLSR(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                        validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                        diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopnLSTMSR(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                     validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                     diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopnLSTMSTD(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                      validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                      diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def TrainLoopnLSTM(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data,
                   validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1,
                   diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
...
def LSTM_combos(ticker, loc, modelsloc, plotsloc, dataloc, etflistloc, vl_later=True, lrg=0.0001, lrd=0.0001,
                n_epochs=500, ngrad=100, h=1, l=10, pred=1, ngpu=1, tanh_coeff=100, tr=0.8, vl=0.1, z_dim=32, hid_d=64,
    ...
    gen, gen_opt, alpha, beta, gamma, delta = GradientCheckLSTM(ticker, gen, gen_opt, ngrad, train_data, batch_size,
                                                                hid_d, hid_g, z_dim, lrd, lrg, h, l, pred, diter,
                                                                tanh_coeff, device, plot)
    ...
    print("PnL")
    losstype = "PnL"
    genPnL, gen_optPnL = TrainLoopnLSTMPnL(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                                           checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g,
                                           z_dim, lrd, lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnL.state_dict()}, f_name + "PnL_lstm_checkpoint.pth")
    df_temp, PnL_test[0], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genPnL, test_data, validation_data, h, l,
                                                              pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, "PnL LSTM",
                                                              0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    print("PnL STD")
    losstype = "PnL STD"
    genPnLMSESTD, gen_optPnLMSESTD = TrainLoopnLSTMPnLSTD(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                                                          checkpoint_epoch, train_data, validation_data, batch_size,
    ...
    df_temp, PnL_test[1], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genPnLMSESTD, test_data, validation_data, h,
                                                              l, pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs,
                                                              "PnL STD LSTM", 0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    print("PnL SR")
    losstype = "PnL SR"
    genPnLMSESR, gen_optPnLMSESR = TrainLoopnLSTMPnLSR(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                                                       checkpoint_epoch, train_data, validation_data, batch_size, hid_d,
                                                       hid_g, z_dim, lrd, lrg, h, l, pred, diter, tanh_coeff, device,
                                                       plot)
    torch.save({'g_state_dict': genPnLMSESR.state_dict()}, f_name + "PnLMSESR_lstm_checkpoint.pth")
    df_temp, PnL_test[2], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genPnLMSESR, test_data, validation_data, h,
                                                              l, pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs,
                                                              "PnL SR LSTM", 0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    print("STD")
    losstype = "STD"
    genPnLSR, gen_optPnLMSESR = TrainLoopnLSTMSTD(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs,
                                                  checkpoint_epoch, train_data, validation_data, batch_size, hid_d,
                                                  hid_g, z_dim, lrd, lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSR.state_dict()}, f_name + "STD_lstm_checkpoint.pth")
    df_temp, PnL_test[3], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genPnLSR, test_data, validation_data, h, l,
                                                              pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, "STD LSTM",
                                                              0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    print("SR")
    losstype = "SR"
    genSR, gen_optSR = TrainLoopnLSTMSR(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                                        train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lrd, lrg, h, l,
                                        pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSR.state_dict()}, f_name + "SR_lstm_checkpoint.pth")
    df_temp, PnL_test[4], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genSR, test_data, validation_data, h, l,
                                                              pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, "SR LSTM",
                                                              0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    print("MSE")
    losstype = "MSE"
    genMSE, gen_optMSE = TrainLoopnLSTM(gen, gen_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch,
                                        train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lrd, lrg, h, l,
                                        pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genMSE.state_dict()}, f_name + "MSE_lstm_checkpoint.pth")
    df_temp, PnL_test[5], PnL_even, PnL_odd = Evaluation2LSTM(ticker, freq, genMSE, test_data, validation_data, h, l,
                                                              pred, hid_d, hid_g, z_dim, lrg, lrd, n_epochs, "MSE", 0,
                                                              device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
...
def FinGAN_universal(tickers1, other, loc, modelsloc, plotsloc, dataloc, etflistloc, vl_later=True, lrg=0.0001,
                     lrd=0.0001, n_epochs=500, ngrad=100, h=1, l=10, pred=1, ngpu=1, tanh_coeff=100, tr=0.8, vl=0.1,
                     z_dim=32, hid_d=64, hid_g=8, checkpoint_epoch=20, batch_size=100, diter=1, plot=False, freq=2):
    """
    ...
    device = torch.device("cuda:0" if (torch.cuda.is_available() and ngpu > 0) else "cpu")
    ticker = tickers1[0]
    train_data, val_data, test_data, dates_dt = split_train_val_test(ticker, dataloc, etflistloc, tr, vl, h, l, pred,
                                                                     plot)
    ...
    genPnL, discPnL, gen_optPnL, disc_optPnL = TrainLoopMainPnLnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  gamma, delta, n_epochs, checkpoint_epoch, train_data,
                                                                  validation_data[0], batch_size, hid_d, hid_g, z_dim,
                                                                  lrd, lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnL.state_dict()}, f_name + "PnL_generator_checkpoint.pth")
    df_temp, PnLs_test[0], PnLs_val[0], means_test[0], means_val[0] = Evaluation3(tickers, freq, genPnL, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "PnL", 0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    ...
    genPnLMSE, discPnLMSE, gen_optPnLMSE, disc_optPnLMSE = TrainLoopMainPnLMSEnv(gen, disc, gen_opt, disc_opt,
                                                                                 criterion, alpha, beta, gamma, delta,
                                                                                 n_epochs, checkpoint_epoch, train_data,
                                                                                 validation_data[0], batch_size, hid_d,
                                                                                 hid_g, z_dim, lrd, lrg, h, l, pred,
                                                                                 diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLMSE.state_dict()}, f_name + "PnLMSE_generator_checkpoint.pth")
    df_temp, PnLs_test[1], PnLs_val[1], means_test[1], means_val[1] = Evaluation3(tickers, freq, genPnLMSE, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "PnL MSE", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "PnL MSE"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genPnLMSESTD, discPnLMSESTD, gen_optPnLMSESTD, disc_optPnLMSESTD = TrainLoopMainPnLMSESTDnv(gen, disc, gen_opt,
                                                                                                ...
                                                                                                plot)
    torch.save({'g_state_dict': genPnLMSESTD.state_dict()}, f_name + "PnLMSESTD_generator_checkpoint.pth")
    df_temp, PnLs_test[2], PnLs_val[2], means_test[2], means_val[2] = Evaluation3(tickers, freq, genPnLMSESTD,
                                                                                  test_data, validation_data, h, l,
                                                                                  pred, hid_d, hid_g, z_dim, lrg, lrd,
                                                                                  n_epochs, "PnL MSE STD", 0, device,
                                                                                  plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "PnL MSE STD"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genPnLMSESR, discPnLMSESR, gen_optPnLMSESR, disc_optPnLMSESR = TrainLoopMainPnLMSESRnv(gen, disc, gen_opt, disc_opt,
                                                                                           ...
                                                                                           checkpoint_epoch, train_data,
                                                                                           validation_data[0],
                                                                                           batch_size, hid_d, hid_g,
                                                                                           z_dim, lrd, lrg, h, l, pred,
                                                                                           diter, tanh_coeff, device,
                                                                                           plot)
    torch.save({'g_state_dict': genPnLMSESR.state_dict()}, f_name + "PnLMSESR_generator_checkpoint.pth")
    df_temp, PnLs_test[3], PnLs_val[3], means_test[3], means_val[3] = Evaluation3(tickers, freq, genPnLMSESR, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "PnL MSE SR", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "PnL MSE SR"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genPnLSR, discPnLSR, gen_optPnLSR, disc_optPnLSR = TrainLoopMainPnLSRnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            ...
                                                                            checkpoint_epoch, train_data,
                                                                            validation_data[0], batch_size, hid_d,
                                                                            hid_g, z_dim, lrd, lrg, h, l, pred, diter,
                                                                            tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSR.state_dict()}, f_name + "PnLSR_generator_checkpoint.pth")
    df_temp, PnLs_test[4], PnLs_val[4], means_test[4], means_val[4] = Evaluation3(tickers, freq, genPnLSR, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "PnL SR", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "PnL SR"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genPnLSTD, discPnLSTD, gen_optPnLSTD, disc_optPnLSTD = TrainLoopMainPnLSTDnv(gen, disc, gen_opt, disc_opt,
                                                                                 ...
                                                                                 diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSR.state_dict()}, f_name + "PnLSTD_generator_checkpoint.pth")
    df_temp, PnLs_test[5], PnLs_val[5], means_test[5], means_val[5] = Evaluation3(tickers, freq, genPnLSTD, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "PnL STD", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "PnL STD"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genSR, discSR, gen_optSR, disc_optSR = TrainLoopMainSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                             gamma, delta, n_epochs, checkpoint_epoch, train_data,
                                                             validation_data[0], batch_size, hid_d, hid_g, z_dim, lrd,
                                                             lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSR.state_dict()}, f_name + "SR_generator_checkpoint.pth")
    df_temp, PnLs_test[6], PnLs_val[6], means_test[6], means_val[6] = Evaluation3(tickers, freq, genSR, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "SR", 0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "SR"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genSRMSE, discSRMSE, gen_optSRMSE, disc_optSRMSE = TrainLoopMainSRMSEnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            ...
                                                                            tanh_coeff, device, plot)
    torch.save({'g_state_dict': genSRMSE.state_dict()}, f_name + "SRMSE_generator_checkpoint.pth")
    df_temp, PnLs_test[7], PnLs_val[7], means_test[7], means_val[7] = Evaluation3(tickers, freq, genSRMSE, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "SR MSE", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "SR MSE"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genMSE, discMSE, gen_optMSE, disc_optMSE = TrainLoopMainMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  ...
                                                                  lrd, lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genMSE.state_dict()}, f_name + "MSE_generator_checkpoint.pth")
    df_temp, PnLs_test[8], PnLs_val[8], means_test[8], means_val[8] = Evaluation3(tickers, freq, genMSE, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "MSE", 0, device, plotsloc, f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "MSE"
    plt.figure(" portfolio cumPnL- " + f_name)
    ...
    genFG, discFG, gen_optFG, disc_optFG = TrainLoopForGAN(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma,
                                                           ...
                                                           lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genFG.state_dict()}, f_name + "ForGAN_generator_checkpoint.pth")
    df_temp, PnLs_test[9], PnLs_val[9], means_test[9], means_val[9] = Evaluation3(tickers, freq, genFG, test_data,
                                                                                  validation_data, h, l, pred, hid_d,
                                                                                  hid_g, z_dim, lrg, lrd, n_epochs,
                                                                                  "ForGAN", 0, device, plotsloc,
                                                                                  f_name1)
    results_df = pd.concat([results_df, df_temp], ignore_index=True)
    losstype = "BCE"
    plt.figure(" portfolio cumPnL- " + f_name)
